...
This can be resolved by *namespacing* (see `Can MotoROS2 run in a namespace? <#can-motoros2-run-in-a-namespace>`__), which causes each controller to use unique names for the resources it makes available (ie: topics, services and actions).
...
MotoROS2 will include all joints from all groups in ``JointState`` messages, and all joints from all groups are expected to be present in ``FollowJointTrajectory`` action goals (see also `No support for partial goals <../README.md#no-support-for-partial-goals>`__).
...
Instead, the `installation procedure <../README.md#installation>`__ relies on pre-built binaries made available by Yaskawa Motoman (on the `Releases page <../README.md#downloading-the-files>`__).
...
**Note**: the `micro-ROS Agent <../README.md#the-micro-ros-agent>`__ is *not* automatically started, and must be active for MotoROS2 to be able to interact with regular ROS 2 applications.
...
Make sure to provide the required/expected arguments (refer to `micro-ROS Agent <../README.md#the-micro-ros-agent>`__), or reuse the ``.launch.py`` provided by the ``micro_ros_agent`` package itself.
...
Using the Agent’s `Docker image <../README.md#using-docker-linux-only>`__ makes this quite easy on Linux.
...
Follow `Updating the configuration <../README.md#updating-the-configuration>`__ to propagate this change to MotoROS2.
...
Refer to `Usage - With MoveIt <../README.md#with-moveit>`__ for information on how to update the configuration of a MoveIt configuration package to work with MotoROS2.
...
Refer to `No support for asynchronous motion <../README.md#no-support-for-asynchronous-motion>`__ for some more information.
...
After changing the configuration, the `changes will need to be propagated to the Yaskawa controller <../README.md#updating-the-configuration>`__.
...
Future development may introduce a mode which could be made compatible with ``ros2_control``, but this is currently not on the `roadmap <../README.md#provisional-roadmap>`__.
